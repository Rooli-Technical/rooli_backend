...
export class AdminDashboardService {
  ...
  async getDashboardData(dateFrom?: string, dateTo?: string): Promise<DashboardAnalytics> {
    const now         = new Date();
    ...
    if (isNaN(from.getTime())) {
      throw new BadRequestException('Invalid dateFrom. Use ISO 8601 format e.g. 2025-01-01');
    }
    if (isNaN(to.getTime())) {
      throw new BadRequestException('Invalid dateTo. Use ISO 8601 format e.g. 2025-06-30');
    }
    if (from > to) {
