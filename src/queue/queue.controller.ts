...
export class QueueSlotController {
  ...
  @ApiOperation({
    ...
    description:
      'Creates a batch of slots based on provided times and days while checking tier limits.',
  })
  ...
  @Get('queue/next')
  @ApiOperation({
    summary: 'Get next available slot time (does not schedule anything)',
  })
  @ApiParam({ name: 'workspaceId', example: 'ws_123' })
  @ApiQuery({
    name: 'platform',
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      'ISO-8601 start time (defaults to now) Optionally specify a "from" time to find the next slot after that time instead of now.',
  ...
  async next(
    ...
    const next = await this.service.getNextAvailableSlotTime(
      workspaceId,
      platform ?? null,
      from,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Resets all future SCHEDULED or QUEUED posts back to DRAFT status.',
  })
  @ApiParam({ name: 'workspaceId', description: 'The CUID of the workspace' })
  @ApiQuery({
    name: 'platform',
    ...
    required: false,
    description:
      'Filter by platform (e.g. LINKEDIN). If omitted, clears all platforms.',
  })
