...
export class AiService {
  ...
  async generateCaption(workspaceId: string, dto: GenerateCaptionDto) {
    ...
    return this.executeWithQuota(workspaceId, cost, async () => {
      ...
      const system = this.promptBuilder.buildSystemPrompt({ brandKit, platform, depth: limits.brandKitDepth });
      const user = this.promptBuilder.buildUserPrompt(dto.prompt);

      const result = await textProvider.generateText({
        system, user, model, temperature: 0.7, maxTokens: 500, responseFormat: 'text',
      });

      const log = await this.logGeneration({
        workspaceId, organizationId: ctx.organizationId, type: AiType.TEXT, feature: AiFeature.CAPTION,
        provider, creditCost: cost, // 🚨 Logging = Billing
        model: result.model ?? model, prompt: system + '\n\nUSER:\n' + user,
        input: { platform, maxChars: dto.maxChars, tone: dto.tone, brandKitId },
        output: { text: result.text }, usage: result.usage, brandKitId, postId: null,
      });
  ...
  async generatePlatformVariants(workspaceId: string, dto: GenerateVariantsDto) {
    ...
    return this.executeWithQuota(workspaceId, cost, async () => {
      ...
      if (dto.platforms.length > limits.maxPlatforms) {
        throw new ForbiddenException(`Your ${plan} plan allows max ${limits.maxPlatforms} platforms at once.`);
      }

      const variantsPerPlatform = Math.min(dto.variantsPerPlatform ?? 3, limits.maxVariants);
      const { brandKit, brandKitId } = await this.resolveBrandKit(workspaceId, dto.brandKitId);
      ...
      const outputs = await Promise.all(
        dto.platforms.map(async (platform) => {
          const system = this.promptBuilder.buildSystemPrompt({ brandKit, platform, depth: limits.brandKitDepth });
          const user = `Write ${variantsPerPlatform} engaging posts about: "${dto.prompt}"\nTarget Platform: ${platform}\nReturn JSON ONLY: {"variants": ["..."]}`.trim();

          const res = await textProvider.generateText({ system, user, model, temperature: 0.8, maxTokens: 1000, responseFormat: 'json' });
          const parsed = this.safeJson(res.text);
          const variants = Array.isArray(parsed?.variants) ? parsed.variants.slice(0, variantsPerPlatform) : [];
          
          if (variants.length === 0) {
            const fallback = res.text.split('\n---\n').map(s => s.trim()).filter(Boolean).slice(0, variantsPerPlatform);
      ...
      const log = await this.logGeneration({
        workspaceId, organizationId: ctx.organizationId, type: AiType.TEXT, feature: AiFeature.VARIANTS,
        provider, creditCost: cost, model: outputs[0]?.raw.model ?? model, prompt: null,
        input: { platforms: dto.platforms, variantsPerPlatform, brandKitId, plan },
        output: { variants: outputs.map((o) => ({ platform: o.platform, variants: o.variants })) },
        usage, brandKitId, postId: null,
      });

      return {
        variants: outputs.map((o) => ({ platform: o.platform, variants: o.variants })),
        usage, generationId: log.id,
      };
    });
  }
  ...
  async repurposeContent(workspaceId: string, dto: RepurposeContentDto) {
    ...
    await this.planAccessService.ensureFeatureAccess(ctx.organizationId, 'repurposeContent'); // Or whatever the feature flag is in your DB
    
    if (!dto.sourceUrl && !dto.sourceText) throw new BadRequestException('Source URL or text required.');

    const cost = this.getFeatureCost(AiFeature.REPURPOSE, 1);

    return this.executeWithQuota(workspaceId, cost, async () => {
      let sourceText = dto.sourceText;
      if (dto.sourceUrl) sourceText = await this.scraper.scrapeUrl(dto.sourceUrl);
      ...
      const system = this.promptBuilder.buildSystemPrompt({ brandKit, platform: dto.targetPlatform, depth: limits.brandKitDepth });
      const user = this.promptBuilder.buildUserPrompt(`SOURCE CONTENT: ${sourceText}\nTASK: Transform into high-quality ${dto.targetPlatform} post.`);

      const result = await textProvider.generateText({ system, user, model: limits.allowedModels[0], temperature: 0.6 });

      await this.logGeneration({
        workspaceId, organizationId: ctx.organizationId, type: AiType.TEXT, feature: AiFeature.REPURPOSE,
        provider, creditCost: cost, model: result.model || limits.allowedModels[0], prompt: system,
  ...
  async generateBulk(workspaceId: string, dto: BulkGenerateDto) {
    ...
    // 🚨 Check feature access BEFORE charging quota
    await this.planAccessService.ensureFeatureAccess(ctx.organizationId, 'bulkAI');
    ...
    return this.executeWithQuota(workspaceId, cost, async () => {
      ...
      const system = this.promptBuilder.buildSystemPrompt({ brandKit, platform: dto.platforms[0], depth: limits.brandKitDepth });
      const user = this.promptBuilder.buildUserPrompt(`TOPIC: ${dto.topic}\nQUANTITY: Generate ${dto.count} distinct social media posts.\nReturn ONLY JSON: {"posts": [{"content": "...", "suggestedDay": 1}]}`);

      const result = await textProvider.generateText({ system, user, model: limits.allowedModels[0], temperature: 0.8, responseFormat: 'json' });
      const parsed = this.safeJson(result.text);
      const posts = parsed?.posts || [];

      await this.logGeneration({
        workspaceId, organizationId: ctx.organizationId, type: AiType.TEXT, feature: AiFeature.BULK,
        provider, creditCost: cost, model: result.model || limits.allowedModels[0], prompt: system,
        input: { topic: dto.topic, requestedCount: dto.count }, output: { countGenerated: posts.length },
        usage: result.usage, brandKitId, postId: null,
      });
  ...
  async generatePostImage(workspaceId: string, userId: string, dto: { prompt: string; style?: string }) {
    ...
    return this.executeWithQuota(workspaceId, cost, async () => {
      ...
      const mediaFile = await this.postMedia.uploadAiGeneratedBuffer(userId, workspaceId, imageBuffer, dto.prompt);

      await this.logGeneration({
        workspaceId, organizationId: ctx.organizationId, type: AiType.IMAGE, feature: AiFeature.IMAGE,
        provider: AiProvider.HUGGINGFACE, creditCost: cost, model: 'black-forest-labs/FLUX.1-schnell',
        prompt: enhancedPrompt, input: { originalPrompt: dto.prompt, style: dto.style },
        output: { mediaId: mediaFile.id, url: mediaFile.url }, usage: { inputTokens: 0, outputTokens: 0, totalTokens: 100 },
        brandKitId: null, postId: null,
      });
  ...
  async generateHashtags(workspaceId: string, prompt: string) {
    ...
    return this.executeWithQuota(workspaceId, cost, async () => {
      ...
      const result = await textProvider.generateText({ system, user, model: 'mistralai/Mistral-7B-Instruct-v0.3', responseFormat: 'json' });
      const parsed = this.safeJson(result.text);
  ...
  async optimizeContent(workspaceId: string, content: string, instruction: string) {
    const cost = this.getFeatureCost(AiFeature.CAPTION, 1);
    ...
    return this.executeWithQuota(workspaceId, cost, async () => {
      ...
      const result = await textProvider.generateText({ system, user, model: 'mistralai/Mistral-7B-Instruct-v0.3', temperature: 0.7 });
      
      return { rewritten: result.text };
    });
  }

  async generateHolidayPost(workspaceId: string, dto: { holidayName: string; platform: 'TWITTER' | 'X' | 'LINKEDIN' | 'INSTAGRAM' | 'FACEBOOK' | 'TIKTOK'; brandKitId?: string; maxChars?: number; }) {
    ...
    return this.executeWithQuota(workspaceId, cost, async () => {
      ...
      const baseSystem = this.promptBuilder.buildSystemPrompt({ brandKit, platform: dto.platform, maxChars: dto.maxChars, depth: limits.brandKitDepth });
      const holidaySystem = this.promptBuilder.buildHolidayAddonPrompt(holidayName);
      const system = `${baseSystem}\n\n---\n${holidaySystem}`;
      const user = this.promptBuilder.buildUserPrompt(`Write a ${dto.platform} post for ${holidayName}.`);
      ...
      const result = await textProvider.generateText({ system, user, model, temperature: 0.8, maxTokens: 600, responseFormat: 'text' });

      const log = await this.logGeneration({
        workspaceId, organizationId: ctx.organizationId, type: AiType.TEXT, feature: AiFeature.HOLIDAY_POST,
        provider: providerKey, creditCost: cost, model: result.model ?? model, prompt: system + '\n\nUSER:\n' + user,
        input: { holidayName, platform: dto.platform, brandKitId }, output: { text: result.text },
        usage: result.usage, brandKitId, postId: null,
      });

      return {
        text: result.text, provider: result.provider ?? providerKey, model: result.model ?? model,
        usage: result.usage ?? null, generationId: log.id,
      };
    });
  }
